...
PERF_RESULT v2_results_context_once[MU_ENUM_VALUE_COUNT_WITHOUT_INVALID(PERF_TEST_LIST)];
PERF_RESULT v2_results_context_every_time[MU_ENUM_VALUE_COUNT_WITHOUT_INVALID(PERF_TEST_LIST)];
...
int main(void)
{
    ...
    // now have the test run with context created once
    start_time = timer_global_get_elapsed_us();
    iterations = 0;
    ...
    // now have the test run with context created every time
    start_time = timer_global_get_elapsed_us();
    iterations = 0;
    ...
    v2_results_context_every_time[PERF_TEST_LOG_CRITICAL].log_count = iterations;
    v2_results_context_every_time[PERF_TEST_LOG_CRITICAL].time = current_time - start_time;

#define PRINT_RESULTS(title, results_var) \
    (void)printf(title " %" PRI_MU_ENUM " %" PRIu64 " logs in %lf s (%" PRIu64 " us), %lf logs/us\r\n", MU_ENUM_VALUE(PERF_TEST_LIST, i), results_var[i].log_count, results_var[i].time / 1000000, (uint64_t)results_var[i].time, (double)results_var[i].log_count / (results_var[i].time / 1000000));

    for (int i = 0; i < MU_ENUM_VALUE_COUNT_WITHOUT_INVALID(PERF_TEST_LIST); i++)
    {
        PRINT_RESULTS("v1", v1_results);
        PRINT_RESULTS("v2", v2_results);
        PRINT_RESULTS("v2 with context created once", v2_results_context_once);
        PRINT_RESULTS("v2 with context created every time", v2_results_context_every_time);
    }
}
